...
- We observed systemic issues like the "Token Snowball" effect and, most critically, a tendency for AI systems to "fail expensively." This behavior—where AI systems get stuck in resource-intensive loops on unsolvable problems—not only hinders their real-world use but also poses a significant barrier to effective RL training by making each failed "rollout" prohibitively costly.
