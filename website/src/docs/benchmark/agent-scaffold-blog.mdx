---
title: 'Re-Evaluating SWE Agent Scaffold '
date: 2025-07-18
...
We present our findings in the figures and tables below. Table 1 provides an overall comparison of agent scaffolds and models across a number of metrics. Table 2 compares the scaffold dynamics and performance between resolved and unresolved cases. Figure 1 gives insights about token consumption and resolve rate of different agent scaffolds. Figure 2 compares the quantity of LLM calls per issue instance. Figure 3 shows where the agent scaffold spent time during the issue resolution process. Figure 4 compares token consumption between successfully resolved and failed cases. Figure 5 compares scaffold performance based on the five efficiency metrics presented above (all measured as normalized AUC except for resolve rate): token efficiency, cost efficiency, CPU efficiency, and inference efficiency. Figure 2 presents a comparison between the number of LLM calls. Figure 6 observes the relationship between number of input tokens and number of LLM calls, showcasing the Token Snowball effect.
...
![Figure 5](/SWE-Effi/imgs/token-snowball-effect.png)

<h6 align="center">Figure 5</h6>
...
#### Observation 4: Failing is Far More Expensive Than Succeeding Not All Agents Fail Equally
