...
Table 1 also compares resource usage between resolved (R) and unresolved (U) issues. We can observe a trend that unresolved issues tend to consume more resources on average (Figure 5), including longer inference time, higher token usage, and more execution steps, indicating inefficiencies in failure cases.
